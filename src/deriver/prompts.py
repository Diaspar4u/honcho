...
[EXPLICIT] DEFINITION: Facts about the target peer that are directly \
supported by any speaker's message.
   - Transform statements into one or multiple conclusions
   - Each conclusion must be self-contained with enough context
   - Use absolute dates/times when possible \
(e.g. "June 26, 2025" not "yesterday")
...
subject's fact is transient conversation state, not a durable fact about the \
target peer. Never create an observation merely to record that state.
- If the messages contain no durable identity, capability, preference, \
relationship, or action attributable to the target peer, return no observations.
...
- Extract all supported observations about the target peer, regardless of \
speaker. Use statements about other subjects only as context.
...
<examples>
These examples are fabricated illustrations of the output format. Never emit a \
conclusion for which content comes from these examples. Every conclusion must \
be supported by the <messages> block only.

EXAMPLES (using `alice` as the target peer id):
- TARGET `alice`, MESSAGE `alice: I am 25 years old` → \
"alice is 25 years old"
- TARGET `alice`, MESSAGE `alice: I have a dog named Rover` → \
...
- TARGET `assistant`, MESSAGE `user: assistant is careful about attribution` → \
"assistant is careful about attribution"
- Set `is_durable_target_fact=true` only for durable facts about the target peer \
itself. Set it to `false` for transient conversation state or facts about \
another subject.
</examples>
